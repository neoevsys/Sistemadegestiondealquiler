    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">Editar Instalación</h1>
                    <p class="mt-2 text-gray-600">Modificar {{ $instalacion->nombre }} - {{ $centro->nombre }}</p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <form action="{{ route('propietario.centros.instalaciones.update', [$centro, $instalacion]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="number" 
                                   ...
                                   min="0"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                   value="{{ old('precio_por_hora', $instalacion->precio_por_hora) }}" 
                                   required>
                        ...
                        <div>
                            ...
                            <input type="number" 
                                   ...
                                   min="1"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                   value="{{ old('capacidad_maxima', $instalacion->capacidad_maxima) }}" 
                                   required>
                        ...
                        <div>
                            ...
                            <select name="superficie" 
                                ...
                                @foreach($superficies as $superficie)
                                    <option value="{{ $superficie }}" {{ old('superficie', $instalacion->superficie) == $superficie ? 'selected' : '' }}>
                                        {{ $superficie }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="text" 
                                   ...
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                   value="{{ old('dimensiones', $instalacion->dimensiones) }}">
                            @error('dimensiones')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select name="estado_id" 
                                ...
                                @foreach($estadosInstalacion as $estado)
                                    <option value="{{ $estado->id }}" {{ old('estado_id', $instalacion->estado_id) == $estado->id ? 'selected' : '' }}>
                    ...
                    <div class="mt-6">
                        ...
                        <textarea name="descripcion" 
                                  ...
                                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('descripcion', $instalacion->descripcion) }}</textarea>
                        @error('descripcion')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        @foreach($tiposDeporte as $deporte)
                            <div class="flex items-center">
                                <input type="checkbox" 
                                       ...
                                       {{ in_array($deporte->id, old('tipos_deporte', $instalacion->tiposDeporte->pluck('id')->toArray())) ? 'checked' : '' }}>
                ...
                @if($instalacion->foto_principal || ($instalacion->fotos_adicionales && count($instalacion->fotos_adicionales) > 0))
                    <div class="mb-8">
                        <h2 class="text-xl font-semibold text-gray-900 mb-4">Fotos Actuales</h2>
                ...
                <!-- Nuevas Fotos -->
                <div class="mb-8">
                    <h2 class="text-xl font-semibold text-gray-900 mb-4">Actualizar Fotos</h2>
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="foto_principal" class="block text-sm font-medium text-gray-700 mb-2">Nueva Foto Principal</label>
                            <input type="file" 
                                   name="foto_principal" 
                                   id="foto_principal" 
                                   accept="image/*"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            <p class="mt-1 text-sm text-gray-500">Máximo 5MB. Formatos: JPG, PNG, GIF. Solo seleccionar si deseas cambiar la foto principal.</p>
                        ...
                        <div>
                            <label for="fotos_adicionales" class="block text-sm font-medium text-gray-700 mb-2">Agregar Fotos Adicionales</label>
                            <input type="file" 
                                   ...
                                   multiple
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            <p class="mt-1 text-sm text-gray-500">Puedes seleccionar múltiples archivos. Se agregarán a las fotos existentes.</p>
